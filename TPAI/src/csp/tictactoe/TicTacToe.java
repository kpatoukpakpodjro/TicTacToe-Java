...
public class TicTacToe extends GameSearch {
    ...
    private boolean winCheck(int i1, int i2, int i3, boolean player, TicTacToePosition pos) {
        int b = 0;
        if (player) b = TicTacToePosition.HUMAN;
        else        b = TicTacToePosition.PROGRAM;
        if (pos.board[i1] == b && pos.board[i2] == b && pos.board[i3] == b)  
        	return true;
        return false;
    }
    ...
    public Position [] possibleMoves(Position p, boolean player) {
        ...
        TicTacToePosition pos = (TicTacToePosition)p;
        int i,count = 0;
        for ( i=0; i<9; i++) if (pos.board[i] == 0) count++;
    ...
    public boolean reachedMaxDepth(Position p, int depth) {
        ...
        if (GameSearch.DEBUG) {
            System.out.println("reachedMaxDepth: pos=" + p.toString() + ", depth="+depth+", ret=" + ret);
        }
        return ret;
    }
